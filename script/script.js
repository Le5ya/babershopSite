const API_URL = 'https://pool-silken-face.glitch.me'
...
window.addEventListener('resize', () => {
  if (activeSlide + 2 > sliderItems.length && 
    document.documentElement.offsetWindth > 560) {
...
const labels = data.map((item) => {
  ...
  <span class="radio__label radio__label_spec" style="--bg-image: url(${API__URL}${item.img})">
  ${item.name}
  </span>
  `;
  return label;
...
const labels = data.map((item) => {
  ...
  <span class="radio__label radio__label">
...
const labels = data.map((item) => {
  ...
  <span class="radio__label radio__label">${time}</span>
  `;
  return label;
  });
  wrapper.append(...labels);
...
const initReserve = () => {
  ...
  reserveForm.addEventListener('change', async (event) => {
    ...
    if (target.name ==='service') {
      addDisabled([fieldspec,fielddata,fieldmonth,fieldday,fieldtime, btn]);
      fieldset.innerHTML = '<legend class="reserve__legend">Специалист</legend>';
      addPreload(fieldspec);
      const response = await fetch(`${API__URI}/api?service=${target.value}`);
      const data = await response.json();
      renderSpec(fiedlset, data);
    ...
    if (target.name ==='spec') {
      ...
      const response = await fetch(`${API__URI}/api?spec=${target.value}`);
      const data = await response.json();
    ...
    if (target.name ==='month') {
      ...
      addPreload(fieldday);
      const response = await fetch(`${API__URI}/api?spec=${reserveForm.spec.value}&month=${reserveForm.month.value}`);
      const data = await response.json();
      fieldDay.textContent = '';
      renderDay(fieldday, data, reserveForm.month.value);
      removePreload(fieldday);
      removeDisabled([fielddata, fieldday])
    }
    if (target.name ==='day') {
      addDisabled([fieldtime, btn]);
      addPreload(fieldtime);
      const response = await fetch(`${API__URI}/api?spec=${reserveForm.spec.value}&month=${reserveForm.month.value}&day=${target.value}`);
      const data = await response.json();
      fieldTime.textContent = '';
      renderTime(fieldtime, data, reserveForm.day.value);
      removePreload(fieldtime);
      removeDisabled([fielddata, fieldtime])
    }
    if(target.name === 'time') {
      removeDisabled([btn]);
    }
